...
function getTotalMembersCount()
{
    ...
    $totalMembersQuery = "SELECT COUNT(*) AS totalMembers FROM members where clubname='$clubname'";
...
function getTotalMembershipTypesCount()
{
    global $conn;
    $clubname = $_SESSION['clubname'];
    $totalMembershipTypesQuery = "SELECT COUNT(*) AS totalMembershipTypes FROM membership_types where clubname='$clubname'";
...
function getExpiringSoonCount()
{
    global $conn;
    $clubname = $_SESSION['clubname'];
    $expiringSoonQuery = "SELECT COUNT(*) AS expiringSoon FROM members WHERE clubname='$clubname' and expiry_date BETWEEN CURDATE() AND CURDATE() + INTERVAL 7 DAY";
...
function getTotalRevenueWithCurrency()
{
    ...
    $totalRevenueQuery = "SELECT SUM(total_amount) AS totalRevenue FROM renew r, members m where m.id=r.member_id and m.clubname='$clubname'";
    $totalRevenueResult = $conn->query($totalRevenueQuery);
...
function getNewMembersCount() {
  ...
  $newMembersQuery = "SELECT COUNT(*) AS newMembersCount FROM members WHERE clubname='$clubname' and created_at >= FROM_UNIXTIME($twentyFourHoursAgo)";
...
function getExpiredMembersCount() {
  global $conn;
  $clubname = $_SESSION['clubname'];
  $expiredMembersQuery = "SELECT COUNT(*) AS expiredMembersCount FROM members WHERE clubname='$clubname' and (expiry_date IS NULL OR expiry_date < NOW())";
      ...
      <div class="container-fluid">
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            $clubname = $_SESSION['clubname'];
            $recentMembersQuery = "SELECT * FROM members where clubname='$clubname' ORDER BY created_at DESC LIMIT 4";
...
function getMembershipTypeName($membershipTypeId)
{
    global $conn;
    $clubname = $_SESSION['clubname'];
    $membershipTypeQuery = "SELECT type FROM membership_types WHERE id = $membershipTypeId and clubname='$clubname'";
